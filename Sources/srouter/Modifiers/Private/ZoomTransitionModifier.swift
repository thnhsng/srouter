...
struct ZoomTransitionModifier<Transition: ZoomTransition>: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ...
        content
            .applyIfPresent(nsToUse) { ns, view in
                view.navigationTransition(.zoom(sourceID: transition.sourceID, in: ns))
            }
...
private struct ZoomSourceModifier<T: ZoomTransition>: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ...
        if #available(iOS 18.0, tvOS 18.0, visionOS 2.0, *) {
            content.applyIfPresent(nsToUse) { ns, view in
                view.matchedTransitionSource(id: transition.sourceID, in: ns)
            }
        } else {
            content
        }
#elseif os(macOS)
        // macOS fallback
        return content.applyIfPresent(nsToUse) { ns, view in
            view.matchedGeometryEffect(id: transition.sourceID, in: ns)
        }
#else
        return content
#endif
